...
public class AS2ApplicationMenu extends HLayout {
    ...
    //If we want to show menu in the applicationMenu
    public AS2ApplicationMenu() {
        super();
    ...
    //If we want to show label in the applicationMenu rather than the menu
    public AS2ApplicationMenu(Label labela) {
        super();
        GWT.log("init ApplicationMenu()", null);
        // initialise the Application Menu layout container
        this.setStyleName("crm-ApplicationMenu");
        this.setHeight(APPLICATION_MENU_HEIGHT);
        label = labela;
        label.setStyleName("crm-ApplicationMenu-label");
        label.setHeight(APPLICATION_MENU_HEIGHT);
        label.setWidth100();
//        label.setAlign(Alignment.RIGHT);
    ...
    public Menu addSubMenu(Menu parentMenu, String subMenuName, String menuItemNames, ClickHandler clickHandler) {
        ...
        for (String menuItem1 : menuItems) {
            // remove any whitespace
            String menuItemName = menuItem1.replaceAll("\\W", "");
            ...
            menuItem = new MenuItem(menuItem1, getIcon(menuItem1));
            menuItem.addClickHandler(clickHandler);
            menu.addItem(menuItem);
        }
